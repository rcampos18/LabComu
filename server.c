...
int main(void) {
  ...
  char buf[256], em[12]  ;    // buffer for client data
  int nbytes;
  char rx, tx;
                  ...
                  if (j != listener && j != i) {
                    ...
                    if (j == rx) {
                      ...
                      if (i == A) {
                        ...
                        tx = '3';
                        printf("\t\tAplicando descarga de 77 al Celular \n");
					strcpy(message1, "DUC77");//DUC100 (saldo-100) DSC (saldo)
					if (send(D, message1, strlen(message1) + 1, 0) < 0) //NOTE: we have to do strlen(message) + 1 because we MUST include '\0'
				{
				    puts("Send failed");
				    return 1;
                      }
